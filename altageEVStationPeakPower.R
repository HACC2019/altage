...
for (i in 1:countCycle){
  ...
  meanPower <- c(meanPower, mean(pSubset$Power..kW., trim = 0.0, na.rm = FALSE))
  noOfCycles <- c(noOfCycles, i)
}
...
#Displays Mean Power and Max Power for each Charge Cycle
cycleInfo <- data.frame(noOfCycles,maxPower,meanPower)
